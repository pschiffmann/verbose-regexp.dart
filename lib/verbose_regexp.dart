...
String verbose(String regexp) => regexp.splitMapJoin(_escapeSequence,
...
final RegExp _escapeSequence = RegExp(
    // Group 1 matches an escaped whitespace or `#` character.
    // Remove the escape character, keep the escaped one.
    r'(?:\\(#|\s))'
    r'|'
    // Group 2 filters out stuff that interferes with the other
    // replacements. These strings are written back into the
    // result unaltered:
    r'('
    // - Any escaped character, including backslashes. This way
    //   we count the number of backslashes before a whitespace
    //   or `#` character.
    r'\\[\s\S]'
    r'|'
    // - All character groups, because they might include
...
final RegExp _verboseMatcher = RegExp('#[^\n]*\n?|\\s');
